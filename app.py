...
tab1, tab3, tab4, tab5, tab6, tab7 = st.tabs(["📊 Data Processing",  "🔬 Value Chain Builder", "📈 Analytics & Insights", "📊Market Estimation", "📦EC analysis", "📄Report Generation"])
...
with tab7:
    ...
    Create a detailed PDF-ready report of your entire analysis including value chains, analytics, EXIM data, and market estimation.
    """)
    ...
    if st.button("📄 Generate Report", type="primary", use_container_width=True):
        with st.spinner("📝 Generating comprehensive report..."):
            ...
            if include_value_chain and st.session_state.value_chain_nodes:
                ...
                # Group nodes (application categories)
                group_nodes = [node for node in st.session_state.value_chain_nodes if node.get('is_group', False)]
                if group_nodes:
                    html_content += """
                ...
                html_content += """
                    <div class="chart-placeholder">
                        <p><strong>📊 Value Chain Visualization</strong></p>
                        <p>Interactive value chain diagram available in the application</p>
                    </div>
                    
                    <div class="success-box">
                        <strong>Value Chain Insights:</strong>
                        <ul>
                """
                
                raw_materials = sum(1 for node in st.session_state.value_chain_nodes if node.get('type') == 'Raw Material' and not node.get('is_group', False))
                intermediates = sum(1 for node in st.session_state.value_chain_nodes if node.get('type') == 'Intermediate' and not node.get('is_group', False))
                target_products = sum(1 for node in st.session_state.value_chain_nodes if node.get('type') == 'Target Product' and not node.get('is_group', False))
                
                html_content += f"""
                            <li><strong>Raw Materials:</strong> {raw_materials} nodes</li>
                            <li><strong>Intermediates:</strong> {intermediates} nodes</li>
                            <li><strong>Target Products:</strong> {target_products} nodes</li>
                            <li><strong>Application Categories:</strong> {len(group_nodes)} categories</li>
                            <li><strong>Total Connections:</strong> {len(st.session_state.value_chain_edges)} edges</li>
                        </ul>
                    </div>
                </div>
                <div class="page-break"></div>
            ...
            # Analytics & Insights with actual data from Tab 4
            if include_analytics and st.session_state.saved_datasets:
                html_content += f"""
                <div class="section">
                    <h2>{current_section}. Analytics & Insights</h2>
                    <p>Comprehensive analytics derived from processed trade data, including quarterly trends, geographic distribution, and supply chain analysis.</p>
                """
                current_section += 1
                
                # Process the first dataset for analytics
                ...
                if df_processed is not None and not error:
                    ...
                    if 'Standardized Name' in df_processed.columns:
                        unique_products = df_processed['Standardized Name'].nunique()
                        html_content += f"""
                        <div class="metric-box">
                            <span class="value">{unique_products}</span>
                    ...
                    <div class="chart-placeholder">
                        <p><strong>📈 Quarterly Volume & Price Chart</strong></p>
                        <p>Dual-axis chart showing volume bars and price trend line (available in application)</p>
                    </div>
                    """
                    ...
                    if country_col and country_col in df_processed.columns:
                        ...
                        for idx, row in geo_data.iterrows():
                            ...
                            rank = geo_data.index.get_loc(idx) + 1
                            html_content += f"""
                                <tr>
                        ...
                        <div class="chart-placeholder">
                            <p><strong>🌍 Geographic Distribution Map</strong></p>
                            <p>Choropleth world map showing volume distribution by country (available in application)</p>
                        </div>
                        """
                    
                    # Supply Chain Analysis if available
                    supplier_col = df_processed.attrs.get('supplier_col')
                    buyer_col = df_processed.attrs.get('buyer_col')
                    
                    if supplier_col and buyer_col and supplier_col in df_processed.columns and buyer_col in df_processed.columns:
                        # Get top suppliers and buyers
                        top_suppliers = df_processed.groupby(supplier_col)['Quantity_MT'].sum().sort_values(ascending=False).head(5)
                        top_buyers = df_processed.groupby(buyer_col)['Quantity_MT'].sum().sort_values(ascending=False).head(5)
                        ...
                        <table>
                            <thead>
                                <tr>
                                    <th colspan="2" style="text-align: center; background: #fb923c;">Top 5 Suppliers</th>
                                    <th colspan="2" style="text-align: center; background: #10b981;">Top 5 Buyers</th>
                                </tr>
                                <tr>
                                    <th>Rank</th>
                                    <th>Supplier</th>
                        ...
                        suppliers_list = list(top_suppliers.items())
                        buyers_list = list(top_buyers.items())
                        max_rows = max(len(suppliers_list), len(buyers_list))
                        
                        for i in range(max_rows):
                            html_content += "<tr>"
                            
                            if i < len(suppliers_list):
                                supplier, vol = suppliers_list[i]
                                html_content += f"""
                                    <td><strong>{i+1}</strong></td>
                                    <td>{supplier} ({vol:,.2f} MT)</td>
                                """
                            else:
                                html_content += "<td></td><td></td>"
                            
                            if i < len(buyers_list):
                                buyer, vol = buyers_list[i]
                                html_content += f"""
                                    <td><strong>{i+1}</strong></td>
                                    <td>{buyer} ({vol:,.2f} MT)</td>
                                """
                            else:
                                html_content += "<td></td><td></td>"
                        ...
                        available_fys = sorted(df_processed['FY'].dropna().unique())
                        if len(available_fys) > 0:
                            selected_fy = available_fys[-1]  # Latest FY
                            fy_data = df_processed[df_processed['FY'] == selected_fy].copy()
                            
                            flow_data = fy_data.groupby([supplier_col, buyer_col])['Quantity_MT'].sum().reset_index()
                            flow_data.columns = ['Supplier', 'Buyer', 'Volume']
                            top_pairs = flow_data.sort_values('Volume', ascending=False).head(5)
                            
                            html_content += f"""
                            <h3>Top 5 Trading Pairs ({selected_fy})</h3>
                            <table>
                                <thead>
                                    <tr>
                                        <th>Rank</th>
                            ...
                            for idx, row in top_pairs.iterrows():
                                rank = top_pairs.index.get_loc(idx) + 1
                                html_content += f"""
                                    <tr>
                                        <td><strong>{rank}</strong></td>
                                        <td>{row['Supplier']}</td>
                            ...
                            <div class="chart-placeholder">
                                <p><strong>🔗 Supply Chain Flow Diagram</strong></p>
                                <p>Sankey diagram showing interactive flows between suppliers and buyers (available in application)</p>
                            </div>
                            """
                    ...
                    # Calculate growth if multiple quarters
                    if len(quarterly) > 1:
                        first_quarter_vol = quarterly.iloc[0]['Quantity_MT']
                        last_quarter_vol = quarterly.iloc[-1]['Quantity_MT']
                        vol_change = ((last_quarter_vol - first_quarter_vol) / first_quarter_vol * 100) if first_quarter_vol > 0 else 0
                        
                        first_quarter_price = quarterly.iloc[0]['Price_USD_per_kg']
                        last_quarter_price = quarterly.iloc[-1]['Price_USD_per_kg']
                        price_change = ((last_quarter_price - first_quarter_price) / first_quarter_price * 100) if first_quarter_price > 0 else 0
                        
                        html_content += f"""
                            <li><strong>Volume Trend:</strong> {vol_change:+.1f}% change from first to last quarter</li>
                            <li><strong>Price Trend:</strong> {price_change:+.1f}% change from first to last quarter</li>
                        """
                    
                    if country_col and country_col in df_processed.columns:
                        unique_countries = df_processed[country_col].nunique()
                        html_content += f"""
                            <li><strong>Geographic Reach:</strong> Trading with {unique_countries} countries</li>
                        """
                    
                    if supplier_col and supplier_col in df_processed.columns:
                        unique_suppliers = df_processed[supplier_col].nunique()
                        html_content += f"""
                            <li><strong>Supplier Base:</strong> {unique_suppliers} unique suppliers</li>
                        """
                    
                    if buyer_col and buyer_col in df_processed.columns:
                        unique_buyers = df_processed[buyer_col].nunique()
                        html_content += f"""
                            <li><strong>Customer Base:</strong> {unique_buyers} unique buyers</li>
                        """
                    ...
                    <div class="info-box">
                        <strong>Analytics Methodology:</strong>
                        <ul>
                            <li>Only METRIC TON and KILOGRAMS units included in analysis</li>
                            <li>Weighted average price calculated using quantity-weighted formula</li>
                            <li>Financial year based on April-March cycle</li>
                            <li>Geographic analysis based on country of origin/destination</li>
                        </ul>
                    </div>
                """
                else:
                    ...
                    <div class="warning-box">
                        <strong>Note:</strong> Detailed analytics could not be generated for this dataset. Please ensure the dataset has valid Date, Quantity, Unit, and Value columns with MT/KG entries.
                    </div>
                    """
            ...
            if include_exim:
                ...
                <div class="section">
                    ...
                    <p>Detailed Import, Export, and Global trade analysis with financial year breakdowns and weighted average pricing.</p>
                ...
                # Process EXIM data if available
                if st.session_state.saved_datasets:
                    financial_years = [
                        "FY 2020-21", "FY 2021-22", "FY 2022-23", 
                        "FY 2023-24", "FY 2024-25", "FY 2025-26"
                    ...
                    # Add Import, Export, Global sections
                    for section in ['Imports', 'Exports', 'Global']:
                        # Check if we have data for this section
                        section_type = section.rstrip('s')
                        has_data = any(ds['type'] == section_type for ds in st.session_state.saved_datasets.values())
                        
                        if has_data:
                            html_content += f"""
                    <h3>{section}</h3>
                    <p>Comprehensive {section.lower()} analysis showing volume and weighted average pricing across financial years.</p>
                    
                    <div class="chart-placeholder">
                        <p><strong>📊 {section} Data Table</strong></p>
                        <p>Complete EXIM analysis with product-wise breakdowns, relation classifications, and FY-wise metrics</p>
                        <p>Access the full interactive table in the Analytics & Insights tab for sorting, filtering, and detailed analysis</p>
                    </div>
                    """
                
                html_content += """
                    <div class="warning-box">
                        <strong>Note:</strong> For complete EXIM analysis with all products and financial quarters, please refer to the Analytics & Insights tab in the application. The interactive tables provide sorting, filtering, and export capabilities with the following features:
                        <ul>
                            <li>Product-wise quantity and price breakdown</li>
                            <li>Relation to product classification (Product, Downstream, Raw Material, etc.)</li>
                            <li>Financial year-wise comparison (FY 2020-21 to FY 2025-26)</li>
                            <li>Calculation detail views showing weighted average formulas</li>
                            <li>Category-wise summary statistics</li>
                            <li>Export options for Excel and complete reports</li>
                        </ul>
                    </div>
                </div>
                <div class="page-break"></div>
            ...
            if include_market_estimation and st.session_state.downstreams:
                ...
                html_content += f"""
                    <h3>Total Market Demand Estimation</h3>
                    <div class="metric-box">
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr style="background-color: #fef3c7; font-weight: bold;">
                                ...
                                <td><strong>{total_demand:,.2f}</strong></td>
                ...
                html_content += """
                    ...
                    <div class="chart-placeholder">
                        <p><strong>🥧 Market Distribution Pie Chart</strong></p>
                        <p>Visual representation showing percentage contribution of each downstream application to total demand</p>
                    </div>
                    
                    <h3>Top Contributors Analysis</h3>
                """
                ...
                for i, ds in enumerate(sorted_downstreams[:5], 1):
                    contribution_pct = (ds['calculated_demand'] / total_demand * 100) if total_demand > 0 else 0
                    html_content += f"""
                            <tr>
                ...
                html_content += """
                    ...
                    <div class="info-box">
                        ...
                        <p><strong>Target Product Demand = Downstream Demand (MT) × Norm (MT/MT)</strong></p>
                        <p>Where <strong>Norm</strong> represents the consumption ratio (how many MT of target product needed per MT of downstream product)</p>
                        <br>
                        <p><strong>Example:</strong> If Paracetamol demand is 12,500 MT and norm is 0.85, then target product demand = 12,500 × 0.85 = 10,625 MT</p>
                    </div>
                </div>
                <div class="page-break"></div>
                """
            ...
            if include_ec_analysis:
                ...
                <div class="section">
                    ...
                    <p>This section contains information about environmental clearances related to the analyzed chemical products.</p>
                    
                    <div class="warning-box">
                        <strong>Note:</strong> Environmental clearance data is searched on-demand through the EC Analysis tab in the application. Results include:
                        <ul>
                            <li>Company names with EC approvals</li>
                            <li>Production capacities (MT or TPA)</li>
                            <li>EC approval dates</li>
                            <li>Project locations (state/district)</li>
                            <li>Context and product mentions within EC documents</li>
                            <li>Direct links to EC documents (PARIVESH and MoEFCC)</li>
                        </ul>
                        <p>Please use the EC Analysis tab to search for specific products and CAS numbers to get the most current information from verified government sources.</p>
                    </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>https://parivesh.nic.in</td>
                                <td>Central and State EC applications and approvals</td>
                            </tr>
                            <tr>
                                <td><strong>MoEFCC</strong></td>
                                <td>https://environmentclearance.nic.in</td>
                                <td>Ministry of Environment, Forest and Climate Change</td>
                            </tr>
                            <tr>
                                <td><strong>State Portals</strong></td>
                                <td>Various state-specific URLs</td>
                                <td>State-level environmental clearance databases</td>
                            </tr>
                        </tbody>
                    </table>
                    
                    <div class="info-box">
                        <strong>How to Use EC Analysis:</strong>
                        <ol>
                            <li>Navigate to the EC Analysis tab in the application</li>
                            <li>Enter the product name (e.g., Tiafenacil, Aniline)</li>
                            <li>Optionally enter the CAS number for more precise results</li>
                            <li>Click "Search Environmental Clearances"</li>
                            <li>Review the structured results with company names, capacities, dates, and links</li>
                            <li>Download results for further analysis or documentation</li>
                        </ol>
                    </div>
                </div>
                <div class="page-break"></div>
            ...
            html_content += f"""
                <div class="footer">
                    <p>Generated by Aarti Industries Trade Data Standardization System</p>
                    <p>Report Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
                    <p>&copy; {datetime.now().year} {company_name}. All rights reserved.</p>
                    <p style="margin-top: 10px; font-size: 0.8rem;">This report is confidential and intended for internal use only.</p>
                </div>
            </body>
            </html>
            """
            
            # Display success and download options
            st.success("✅ Report generated successfully!")
            ...
            with stat_col1:
                ...
                st.metric("Sections Included", sections_included)
            ...
            with stat_col3:
                total_nodes = len([n for n in st.session_state.value_chain_nodes if not n.get('is_group', False)])
                st.metric("Value Chain Nodes", total_nodes)
            
            with stat_col4:
                total_downstreams = len(st.session_state.downstreams)
                st.metric("Market Applications", total_downstreams)
            
            # Report generation info
            st.markdown("---")
            st.markdown("### ℹ️ Report Information")
            ...
            **Report Details:**
            ...
            with st.expander("💡 Tips for Best Results"):
                ...
                **Viewing the HTML Report:**
                - Open the downloaded HTML file in any modern web browser (Chrome, Firefox, Edge, Safari)
                - The report is self-contained with embedded styling
                - All formatting will be preserved
                
                **Converting to PDF:**
                - Method 1: Use browser's Print to PDF (Ctrl/Cmd + P → Save as PDF)
                - Method 2: Use online HTML to PDF converters
                - Method 3: Use Adobe Acrobat or similar software
                ...
                **Sharing the Report:**
                - HTML format: Easy to view, interactive
                - PDF format: Professional, printable, universally compatible
                
                **Report Customization:**
                - Use the checkboxes above to include/exclude sections
                - Customize company name and author for branding
    ...
    with st.expander("❓ How to Generate Reports", expanded=False):
        ...
        **Step 1: Configure Report**
        - Enter report title (e.g., "Q1 2024 Trade Analysis")
        - Add company/organization name
        - Specify report author name
        - Select report date
        
        **Step 2: Select Sections**
        Choose which sections to include:
        - **Executive Summary:** High-level overview with key metrics
        - **Value Chain Analysis:** Molecular relationships and downstream applications
        - **Analytics & Insights:** Quarterly trends, geographic distribution, supply chain flows
        - **EXIM Analysis:** Import/Export details with weighted pricing
        - **Market Estimation:** Demand calculations by category
        - **Environmental Clearance Analysis:** EC information and sources
        
        **Step 3: Generate**
        - Click "Generate Report" button
        - Wait for processing (typically 5-10 seconds)
        - Preview the report in the expander
        
        **Step 4: Download**
        - Download HTML for browser viewing
        - Convert to PDF for printing/sharing
        
        **What's Included in Each Section:**
        
        **Executive Summary:**
        - Key metrics (datasets, value chain nodes, downstreams)
        - Total data rows processed
        - Quick overview of analysis scope
        
        **Value Chain Analysis:**
        - Complete list of molecules (raw materials, intermediates, target products)
        - Downstream application categories with counts
        - Value chain insights and statistics
        - Connection summary
        
        **Analytics & Insights:**
        - Overview metrics (total volume, weighted avg price, time periods)
        - Quarterly volume and price trends table
        - Top 10 countries by volume with market share
        - Top 5 suppliers and buyers
        - Top 5 trading pairs for latest FY
        - Key analytical insights (growth trends, geographic reach)
        - Chart placeholders for interactive visualizations
        
        **EXIM Analysis:**
        - Calculation methodology explanation
        - Financial year-wise breakdowns
        - Import, Export, and Global trade sections
        - Reference to detailed interactive data
        
        **Market Estimation:**
        - Total market demand summary
        - Category-wise breakdown (Pharma/Agro/Others)
        - Detailed downstream applications table
        - Top 5 contributors analysis
        - Contribution percentages
        - Calculation methodology with examples
        
        **Environmental Clearance Analysis:**
        - Overview of EC search capabilities
        - Data sources (PARIVESH, MoEFCC, State portals)
        - How to use EC Analysis feature
        - Reference to on-demand search feature
        
        **Best Practices:**
        - Include Executive Summary for stakeholders
        - Include all analytical sections for comprehensive reports
        - Generate regularly to track changes over time
        - Customize company name and author for professional branding
        
        **Report Quality:**
        - Professional formatting with Aarti branding
        - Print-ready layout with proper page breaks
        - Responsive design for all screen sizes
        - Comprehensive data with calculations
        - Real analytics data from Tab 4 integrated
        """)
    
    # Additional utilities
    st.markdown("---")
    st.markdown("### 🛠️ Report Utilities")
    ...
    with util_col2:
        if st.button("📋 View Sample Report", use_container_width=True):
            ...
            **Sample Report Structure:**
            ...
            4. Value Chain Analysis with tables and insights
            5. Analytics & Insights with quarterly trends, geographic data, supply chain
            6. EXIM Analysis with methodology
            7. Market Estimation with breakdowns and top contributors
            8. EC Analysis (if selected)
            9. Footer with generation details
            """)
    
    with util_col3:
        if st.button("📧 Sharing Instructions", use_container_width=True):
            st.info("""
            **To share the report:**
            1. Download HTML or convert to PDF
    ...
    with quick_col1:
        ...
        **Before Generating Report:**
        - ✅ Process and save datasets in Tab 1
        - ✅ Build value chains in Tab 3
        - ✅ Review analytics in Tab 4
        - ✅ Add market estimations in Tab 5
        """)
    ...
    with avail_col1:
        if st.session_state.saved_datasets:
            st.success("✅ Datasets Available")
            st.caption(f"{len(st.session_state.saved_datasets)} datasets")
        else:
            st.warning("⚠️ No Datasets")
            st.caption("Process data in Tab 1")
    
    with avail_col2:
        if st.session_state.value_chain_nodes:
            st.success("✅ Value Chain Built")
            st.caption(f"{len([n for n in st.session_state.value_chain_nodes if not n.get('is_group', False)])} nodes")
        else:
            st.warning("⚠️ No Value Chain")
            st.caption("Build chain in Tab 3")
    
    with avail_col3:
        if st.session_state.downstreams:
            st.success("✅ Market Data Ready")
            st.caption(f"{len(st.session_state.downstreams)} applications")
        else:
            st.warning("⚠️ No Market Data")
            st.caption("Add data in Tab 5")
    
    with avail_col4:
        # Check if any EXIM data exists
        has_exim = any(ds['type'] in ['Import', 'Export', 'Global'] for ds in st.session_state.saved_datasets.values()) if st.session_state.saved_datasets else False
        if has_exim:
            st.success("✅ EXIM Data Available")
            st.caption("Trade data processed")
        else:
            st.warning("⚠️ No EXIM Data")
            st.caption("Check Tab 4")
    
    # Footer message
    st.markdown("---")
    st.markdown("""
    <div style='text-align: center; color: #9ca3af; font-size: 0.9rem; padding: 20px;'>
        <p>📄 Professional Report Generator | Aarti Industries Trade Data System</p>
        <p>Generate comprehensive reports combining all your analysis in one professional document</p>
    </div>
    """, unsafe_allow_html=True)
